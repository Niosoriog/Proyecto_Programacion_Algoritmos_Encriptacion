...
def s2b(a):
    ...
    "7":"00110111","8":"00111000","9":"00111001",":":"00111010",";":"00111011","<":"00111100","=":"00111101",">":"00111110","?":"00111111"," ":"01000000","~":"00100000",'"':"00100010","|":"01111100","`":"01100000"}
    codigo_binario = []
    for x in a:
        if x in binario:
...
def b2s(b):
    ...
    "00110111":"7","00111000":"8","00111001":"9","00111010":":","00111011":";","00111100":"<","00111101":"=","00111110":">","00111111":"?","01000000":" ","00100000":"~","00100010":'"',"01111100":"|","01100000":"`"}
    string = ""                                                       
    for y in b:                                                     
        caracter = []                                                                                                       
        if y in caracteres:                                                                                         
...
def desencriptar(mensaje, clave,usuario):
    """
    desencriptar: recibe mensaje encriptado y clave para desencriptar
    :param strign x: mensaje en mayuscula
    :param strign y: clave en maniscula
    :param lista mensaje_binario: codigos binarios del mensaje
    :param lista clave_binario: codigos binarios de la clave
    :param lista mensaje_encriptado: lista de codigos binarios 
    :param string string_encriptada: string despues de realizar la encriptación con el metodo Vernam
    :return: retorna cadena de caracteres desencriptados
    """
    x = mensaje.upper()
    y = clave.lower()
    mensaje_binario = s2b(x)
...
def vernan(mensaje, clave, opcion,usuario):
    """
    vernan: recibe mensaje, clave y la opcion de desencriptar o encriptar
    :param string: mensaje,clave: mensaje y clave de igual longitud
    :param string: cadena: cadena del mensaje encriptado o desencriptado
    :return string: cadena: retorna cadena si el mensaje esta en mayuscula de lo contrario la retorna en minuscula
    """
    clave_inicial = clave
    mensaje, clave = igualar(mensaje, clave)
    cadena = ""
    if opcion == "enc":
        cadena = encriptar(mensaje, clave,clave_inicial,usuario)
    else:
        cadena = desencriptar(mensaje, clave,usuario)
    if mensaje.isupper():
        return cadena
    else:    
